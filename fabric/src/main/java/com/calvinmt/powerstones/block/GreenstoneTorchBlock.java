...
public class GreenstoneTorchBlock
extends TorchBlock {
    ...
    protected boolean shouldUnpower(World world, BlockPos pos, BlockState state) {
        return world.isEmittingGreenstonePower(pos.down(), Direction.DOWN);
    }
    ...
    public void scheduledTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        ...
        if (state.get(LIT).booleanValue()) {
            if (bl) {
                ...
                if (GreenstoneTorchBlock.isBurnedOut(world, pos, true)) {
        ...
        } else if (!bl && !GreenstoneTorchBlock.isBurnedOut(world, pos, false)) {
    ...
    public int getStrongGreenstonePower(BlockState state, BlockView world, BlockPos pos, Direction direction) {
        if (direction == Direction.DOWN) {
            return state.getWeakGreenstonePower(world, pos, direction);
        }
        return 0;
    }
    ...
    public void randomDisplayTick(BlockState state, World world, BlockPos pos, Random random) {
        ...
        world.addParticleClient(new DustParticleEffect(PowerColour.GREEN.getWireColour(15), 1.0F), particlePos.getX(), particlePos.getY(), particlePos.getZ(), 0.0, 0.0, 0.0);
    }
    ...
    private static boolean isBurnedOut(World world2, BlockPos pos, boolean addNew) {
        List<BurnoutEntry> list = BURNOUT_MAP.computeIfAbsent(world2, world -> Lists.newArrayList());
        if (addNew) {
