...
extends TorchBlock {
    ...
    public BluestoneTorchBlock(AbstractBlock.Settings settings) {
        super(settings, new DustParticleEffect(PowerstoneWireBlock.BLUE_COLORS[15].toVector3f(), 1.0f));
    ...
    public void scheduledTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        ...
        if (state.get(LIT).booleanValue()) {
            if (bl) {
                ...
                if (BluestoneTorchBlock.isBurnedOut(world, pos, true)) {
        ...
        } else if (!bl && !BluestoneTorchBlock.isBurnedOut(world, pos, false)) {
    ...
    public int getStrongBluestonePower(BlockState state, BlockView world, BlockPos pos, Direction direction) {
        if (direction == Direction.DOWN) {
            return state.getWeakBluestonePower(world, pos, direction);
        }
        return 0;
    }
